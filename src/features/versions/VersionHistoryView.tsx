...
export function VersionHistoryView({ projectId, onVersionRestored }: VersionHistoryViewProps) {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto p-4 sm:p-6 text-[#f4f4f5]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-4 border-b border-[#27272a]">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="font-mono text-[10px] text-[#0ea5e9]">
              HEAD: {latestVersion ? `v${latestVersion.version_number}.0` : "INITIAL"}
            </span>
          </div>
          <h2 className="text-xl font-bold tracking-tight text-white flex items-center gap-2">
            <GitFork className="h-5 w-5 text-[#0ea5e9]" />
        ...
        <form onSubmit={handleCreateSnapshot} className="flex items-center gap-2">
          <input
            ...
            className="h-9 px-3 rounded-md bg-[#111113] border border-[#27272a] text-xs text-white placeholder-[#71717a] focus:outline-none focus:ring-1 focus:ring-[#0ea5e9] font-mono w-60"
          />
          <Button
            type="submit"
            size="sm"
      ...
      {isCompareOpen && (
        <div className="rounded-xl border border-[#0ea5e9]/40 bg-[#111113] p-5 space-y-4 animate-in fade-in-50 duration-200">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-[#27272a]">
            <div className="flex items-center gap-2">
              <Split className="h-4 w-4 text-[#0ea5e9]" />
      ...
      {loading ? (
        ...
        <div className="relative pl-6 sm:pl-8 space-y-6 before:absolute before:left-3 sm:before:left-4 before:top-3 before:bottom-3 before:w-0.5 before:bg-[#27272a]">
          {versions.map((ver, idx) => {
            ...
            return (
              <div key={ver.id} className="relative group">
                {/* Timeline node icon */}
                <div
                  className={`absolute -left-6 sm:-left-8 top-4 flex h-6 w-6 items-center justify-center rounded-full border-2 bg-[#09090b] transition-transform ${
                    isHead
                      ? "border-[#0ea5e9] text-[#0ea5e9] shadow-[0_0_10px_rgba(14,165,233,0.5)]"
                ...
                <div
                  className={`rounded-xl border transition-colors ${
                    isHead
                      ? "border-[#0ea5e9]/40 bg-[#111113]"
                      : "border-[#27272a] bg-[#111113]/80 hover:border-[#3f3f46]"
                  }`}
                >
                  <div className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {isHead && (
                            <span className="font-mono text-[10px] px-2 py-0.5 rounded bg-[#0ea5e9]/10 text-[#0ea5e9] border border-[#0ea5e9]/30 font-bold">
                              ● CURRENT HEAD
                            </span>
                          )}

                          <span className="font-mono text-[10px] px-1.5 py-0.5 rounded bg-[#18181b] border border-[#27272a] text-[#71717a]">
                            COMMIT {commitHash}
                          </span>
                        </div>
                    ...
                    <div className="flex items-center gap-2 self-end sm:self-center shrink-0 font-mono">
                      <Button
                        ...
                        className="h-8 px-2.5 text-xs border-[#27272a] bg-[#09090b] text-[#a1a1aa] hover:text-white hover:bg-[#18181b]"
                      >
                        <Split className="h-3.5 w-3.5 mr-1 text-[#0ea5e9]" />
                        Compare
                      </Button>
                      ...
                      {!isHead && (
                        <Button
                          ...
                          className="h-8 px-3 text-xs font-semibold bg-[#18181b] hover:bg-[#27272a] text-white border border-[#27272a]"
                        >
                          <RotateCcw className="h-3.5 w-3.5 mr-1 text-[#0ea5e9]" />
                  ...
                  {isInspected && (
                    <div className="px-4 pb-4 pt-3 border-t border-[#27272a] bg-[#09090b]/90 text-xs font-mono">
                      <div className="flex items-center justify-between text-[11px] text-[#a1a1aa] mb-2 pb-1 border-b border-[#27272a]">
                        ...
                        <span className="text-[#0ea5e9]">TIMESTAMP: {ver.created_at}</span>
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                        {ver.architecture_snapshot?.components?.map((c) => (
                          <div
                            key={c.id}
                            className="p-2.5 rounded bg-[#111113] border border-[#27272a]"
                          >
                            ...
                            <div className="text-[10px] text-[#0ea5e9] mt-0.5 truncate">
